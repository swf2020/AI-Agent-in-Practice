...
def main() -> None:
    ...
    # [Fix #8] 使用 min 避免切块少于 5 时 IndexError
    n_test = min(len(chunks), 5)
    count = index_chunks(chunks[:n_test], model, client)  # 只索引前 N 块供测试
    assert count == n_test, f"索引写入数量异常：{count}"
