...
### [1.1 LLM 核心概念（Token、Temperature、上下文窗口）](AI Agent In Practice/01_大模型基础与 API 实战/1.1 LLM核心概念.md)
...
#### [1.2.1 微调技术全景与选型决策](AI Agent In Practice/01_大模型基础与 API 实战/1.2.1 微调技术全景与选型决策.md)
...
#### [1.2.2 LoRA 原理精讲](AI Agent In Practice/01_大模型基础与 API 实战/1.2.2 LoRA 原理精讲.md)
...
#### [1.2.3 QLoRA 原理精讲](AI Agent In Practice/01_大模型基础与 API 实战/1.2.3 QLoRA 原理精讲.md)
...
#### [1.2.4 数据工程：微调数据的准备与质量控制](AI Agent In Practice/01_大模型基础与 API 实战/1.2.4 数据工程：微调数据的准备与质量控制.md)
...
#### [1.2.5 【动手一】用 QLoRA 微调 Qwen2.5-7B 指令模型](AI Agent In Practice/01_大模型基础与 API 实战/1.2.5 【动手一】用 QLoRA 微调 Qwen2.5-7B 指令模型.md)
...
#### [1.2.6 【动手二】微调效果评估与对比实验](AI Agent In Practice/01_大模型基础与 API 实战/1.2.6 【动手二】微调效果评估与对比实验.md)
...
#### [1.2.7 【动手三】基于 Unsloth 的高效微调加速实战](AI Agent In Practice/01_大模型基础与 API 实战/1.2.7 【动手三】基于 Unsloth 的高效微调加速实战.md)
...
### [1.3 Prompt Engineering 实战](AI Agent In Practice/01_大模型基础与 API 实战/1.3 Prompt Engineering 实战.md)
...
### [1.4 主流 API 对比接入](AI Agent In Practice/01_大模型基础与 API 实战/1.4 【动手】统一封装多模型调用层.md)
...
### [2.1 RAG 架构全景](AI Agent In Practice/02_RAG（检索增强生成）/2.1 RAG 架构全景.md)

- Naive RAG → Advanced RAG → Modular RAG 演进路线
- 离线阶段：文档解析 → 切块 → 向量化 → 索引
- 在线阶段：查询理解 → 检索 → 重排 → 生成

### [2.2 Embedding 与向量数据库](AI Agent In Practice/02_RAG（检索增强生成）/2.2 Embedding 与向量数据库.md)
...
### [2.3 检索策略（稠密/稀疏/混合）](AI Agent In Practice/02_RAG（检索增强生成）/2.3 检索策略（稠密 - 稀疏 - 混合）.md)

- 稠密检索：语义向量相似度（cosine / dot product）
- 稀疏检索：BM25 关键词匹配原理与 Elasticsearch 实战
- 混合检索：RRF 融合算法 + 权重调优策略

### [2.4 【动手】从零搭建本地知识库问答系统](AI Agent In Practice/02_RAG（检索增强生成）/2.4 【动手】从零搭建本地知识库问答系统.md)
...
### [2.5 【动手】Advanced RAG：重排序 + 查询改写](AI Agent In Practice/02_RAG（检索增强生成）/2.5 【动手】Advanced RAG：重排序 + 查询改写.md)
...
### [2.6 RAG 评估体系（RAGAS 框架）](AI Agent In Practice/02_RAG（检索增强生成）/2.6 RAG 评估体系（RAGAS 框架）.md)
...
### [3.1 Function Calling 原理与协议](AI Agent In Practice/03_Function Calling MCP 与工具使用/3.1 Function Calling 原理与协议.md)

- JSON Schema 工具定义规范与参数设计最佳实践
- 并行工具调用（Parallel Tool Use）与串行调用的选择
- Structured Output 与 Function Calling 的关系与区别

### [3.2 MCP 协议详解](AI Agent In Practice/03_Function Calling MCP 与工具使用/3.2 MCP 协议详解.md)
...
### [3.3 【动手】给 LLM 接入搜索 / 计算器 / 数据库工具](AI Agent In Practice/03_Function Calling MCP 与工具使用/3.3 【动手】给 LLM 接入搜索 - 计算器 - 数据库工具.md)

- 搜索工具：接入 Tavily / Brave Search API
- 代码执行工具：沙箱环境（E2B / Modal）安全执行
- 数据库工具：Text-to-SQL 生成 + 执行 + 结果解释链路

### [3.4 工具可靠性与错误处理](AI Agent In Practice/03_Function Calling MCP 与工具使用/3.4 工具可靠性与错误处理.md)
...
### [4.1 Agent 定义：感知-规划-行动循环](AI Agent In Practice/04_AI Agent 核心架构/4.1 Agent 定义：感知-规划-行动循环.md)

- 与传统 RPA / 规则引擎的核心区别
- Agent 的四大核心能力：记忆、工具、规划、行动
- Agent 可靠性问题现状：任务完成率 benchmark 解读

### [4.2 ReAct 范式实战](AI Agent In Practice/04_AI Agent 核心架构/4.2 ReAct 范式实战.md)

- Thought → Action → Observation 循环机制详解
- 从零实现一个 ReAct Agent（不依赖框架）
- ReAct 常见失败模式：幻觉行动、死循环、过早停止

### [4.3 Planning 策略（ToT / GoT / Plan-and-Execute）](AI Agent In Practice/04_AI Agent 核心架构/4.3 Planning 策略（ToT - GoT - Plan-and-Execute）.md)

- Tree of Thought：广度优先搜索与最优路径选择
- Plan-and-Execute：先规划全局再逐步执行的分离架构
- 动态重规划：执行结果反馈触发计划调整

### [4.4 记忆系统设计](AI Agent In Practice/04_AI Agent 核心架构/4.4 记忆系统设计.md)
...
### [4.5 【动手】用 LangGraph 构建有状态 Agent](AI Agent In Practice/04_AI Agent 核心架构/4.5 【动手】用 LangGraph 构建有状态 Agent.md)
...
### [5.1 多 Agent 协作模式（层级 / 对等 / 流水线）](AI Agent In Practice/05_Multi-Agent 系统/5.1 多 Agent 协作模式（层级 - 对等 - 流水线）.md)
...
### [5.2 AutoGen 框架实战](AI Agent In Practice/05_Multi-Agent 系统/5.2 AutoGen 框架实战.md)

- ConversableAgent 与 GroupChat 核心抽象
- 代码执行沙箱（Docker executor）安全配置
- Human-in-the-Loop：动态插入人工反馈节点

### [5.3 CrewAI 角色分工实战](AI Agent In Practice/05_Multi-Agent 系统/5.3 CrewAI 角色分工实战.md)

- Agent / Task / Crew 三层抽象建模
- 角色设计最佳实践：goal + backstory 对输出质量的影响
- Process 类型：Sequential vs Hierarchical 对比

### [5.4 【动手】搭建代码生成+审查的双 Agent 系统](AI Agent In Practice/05_Multi-Agent 系统/5.4 【动手】搭建代码生成+审查的双 Agent 系统.md)
...
### [6.1 流式输出与用户体验](AI Agent In Practice/06_生产级落地关键技术/6.1 流式输出与用户体验.md)

- SSE（Server-Sent Events）服务端实现（FastAPI）
- 前端流式渲染：React + ReadableStream 接入
- 中间过程透出：Tool Call 状态实时展示给用户

### [6.2 成本控制与 Token 优化](AI Agent In Practice/06_生产级落地关键技术/6.2 成本控制与 Token 优化.md)

- 请求层：Prompt 压缩 + 动态截断 + 摘要替换
- 模型层：任务路由（复杂 → 强模型，简单 → 弱模型）
- 监控层：按 User / Feature / Session 维度统计成本

### [6.3 缓存策略（Prompt Cache / Semantic Cache）](AI Agent In Practice/06_生产级落地关键技术/6.3 缓存策略（Prompt Cache - Semantic Cache）.md)

- Prompt Cache：Claude / OpenAI 官方缓存机制原理与命中率优化
- Semantic Cache：GPTCache / Zep 语义相似度缓存实战
- 缓存失效策略：TTL vs 内容变更触发失效

### [6.4 安全与对齐（Prompt Injection 防御）](AI Agent In Practice/06_生产级落地关键技术/6.4 安全与对齐（Prompt Injection 防御）.md)

- 攻击类型：直接注入 / 间接注入 / 越狱技巧分类
- 防御策略：输入净化 + 特权提示隔离 + 输出验证
- 【动手】构建一个 Prompt Injection 检测分类器

### [6.5 可观测性（LangSmith / LangFuse）](AI Agent In Practice/06_生产级落地关键技术/6.5 可观测性（LangSmith - LangFuse）.md)

- Trace 采集：完整 Agent 执行链路的 Span 记录
- 核心指标：延迟 P99 / Token 消耗 / 工具调用成功率
- 告警配置：异常延迟与错误率阈值触发告警

### [6.6 【动手】构建带监控的生产级 Agent 服务](AI Agent In Practice/06_生产级落地关键技术/6.6 【动手】构建带监控的生产级 Agent 服务.md)
...
#### [8.1.1 前深度学习时代（2000–2017）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.1.1 前深度学习时代（2000-2017）.md)
...
#### [8.1.2 Transformer 革命（2017–2019）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.1.2 Transformer 革命（2017-2019）.md)
...
#### [8.1.3 规模定律与涌现能力（2020–2022）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.1.3 规模定律与涌现能力（2020-2022）.md)
...
#### [8.1.4 指令对齐与 RLHF 时代（2022–2023）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.1.4 指令对齐与 RLHF 时代（2022-2023）.md)
...
#### [8.1.5 开源生态爆发与长上下文竞赛（2023–2024）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.1.5 开源生态爆发与长上下文竞赛（2023-2024）.md)
...
#### [8.1.6 推理模型范式（2024–2025）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.1.6 推理模型范式（2024-2025）.md)
...
#### [8.2.1 前 LLM Agent 时代（约 1960s–2022）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.2.1 前 LLM Agent 时代（约 1960s-2022）.md)

- 符号 AI 与专家系统：规则驱动的"硬编码"自动化
- 强化学习 Agent：AlphaGo / OpenAI Five 的封闭场景巅峰
- RPA：流程固定时的工业级自动化方案与局限

#### [8.2.2 LLM Agent 萌芽期（2022–2023）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.2.2 LLM Agent 萌芽期（2022-2023）.md)
...
#### [8.2.3 框架与协议的规范化（2023–2024）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.2.3 框架与协议的规范化（2023-2024）.md)
...
#### [8.2.4 Computer Use 与 GUI Agent（2024）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.2.4 Computer Use 与 GUI Agent（2024）.md)
...
#### [8.2.5 Agentic AI 走向生产（2024–2025）](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.2.5 Agentic AI 走向生产（2024-2025）.md)
...
#### [8.3.1 上下文长度：从 512 Token 到无限流式记忆](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.3.1 上下文长度：从 512 Token 到无限流式记忆.md)
...
#### [8.3.2 RAG 技术演进：关键字检索到知识图谱增强](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.3.2 RAG 技术演进：关键字检索到知识图谱增强.md)
...
#### [8.3.3 多模态：从 CLIP 到全模态统一模型](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.3.3 多模态：从 CLIP 到全模态统一模型.md)

- CLIP（2021）奠定视觉语言对齐基础（Radford et al., 2021, arXiv:2103.00020）
- GPT-4o / Gemini 1.5：原生多模态 vs 后融合架构对比
- 音视频 + 代码 + 结构数据的全模态大一统趋势研判

#### [8.3.4 高效推理：从量化压缩到推理加速芯片生态](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.3.4 高效推理：从量化压缩到推理加速芯片生态.md)

- 量化技术：FP16 → INT8 → GPTQ → AWQ → 1-bit（BitNet）
- 推测解码：草稿模型加速主模型生成吞吐
- vLLM / TensorRT-LLM / SGLang：PagedAttention 等关键工程创新梳理

#### [8.3.5 对齐技术：从 RLHF 到 DPO 到宪法 AI](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.3.5 对齐技术：从 RLHF 到 DPO 到宪法 AI.md)
...
#### [8.3.6 微调技术：从全参数到极低成本适配](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.3.6 微调技术：从全参数到极低成本适配.md)
...
#### [8.5.1 模型能力演进预判](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.5.1 模型能力演进预判（2025-2030）.md)
...
#### [8.5.2 Agent 架构演进预判](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.5.2 Agent 架构演进预判.md)
...
#### [8.5.3 技术瓶颈与开放性挑战](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.5.3 技术瓶颈与开放性挑战.md)
...
#### [8.5.4 基础设施与生态演进预判](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.5.4 基础设施与生态演进预判.md)
...
#### [8.5.5 社会影响与监管格局](AI Agent In Practice/08_AI 大模型与 Agent 技术演进全景/8.5.5 社会影响与监管格局.md)
...
### [原理题](AI Agent In Practice/09_面试题真题/01_原理题.md) — 28 题
...
### [工程题](AI Agent In Practice/09_面试题真题/02_工程题.md) — 32 题
...
### [设计题](AI Agent In Practice/09_面试题真题/03_设计题.md) — 20 题
...
### [B. 学习资源导航](AI Agent In Practice/10_附录/B. 学习资源导航.md)

- 必读论文清单（附 arXiv 链接 + 一句话摘要）
- 优质开源项目推荐（附 GitHub Star 与适用场景）
- 持续跟进渠道：Newsletter / X 账号 / Discord 社区

### [C. 术语表](AI Agent In Practice/10_附录/C. 术语表.md)
