...
# 如果使用 Claude（推荐）
uv pip install langchain-anthropic==0.3.3
```

> **Colab 用户**：`!pip install langgraph==0.2.68 langchain-openai==0.2.14 langchain-anthropic==0.3.3 tavily-python==0.5.0` 即可

配置 `.env` 文件：

```bash
# .env
OPENAI_API_KEY=sk-...          # 或使用 Anthropic
ANTHROPIC_API_KEY=sk-ant-...
TAVILY_API_KEY=tvly-...        # 搜索工具，免费额度够用
```
...
### Step 2：定义工具——Agent 的"手"

**目标**：注册 Agent 可调用的工具集。工具是 LangGraph 中唯一能与外部世界交互的接口。
...
### Step 3：构建 Agent Node——LLM 决策中枢

**目标**：实现核心的"大脑"节点——调用绑定了工具的 LLM，输出决策（直接回答 or 调用工具）。
...
### Step 6：Checkpoint 持久化——中断恢复与人工审批
