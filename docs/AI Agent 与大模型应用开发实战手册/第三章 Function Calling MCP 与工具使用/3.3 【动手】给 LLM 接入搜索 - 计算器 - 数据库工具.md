...
本节结束后，你将能够：独立为 LLM 接入三类主流外部工具——实时搜索（Tavily）、安全代码执行沙箱（E2B）、以及 Text-to-SQL 数据库查询链路，并通过 OpenAI Function Calling 协议把它们统一串联成一个可扩展的工具调用框架。
...
> Colab 用户：`!pip install openai tavily-python e2b-code-interpreter sqlalchemy python-dotenv pydantic` 即可
...
**目标**：把三个工具和 OpenAI API 串联成完整的工具调用循环。

OpenAI 的 Function Calling 是一个多轮交互协议：LLM 返回 `tool_calls` → 你执行工具 → 把结果作为 `tool` 角色消息回传 → LLM 继续生成，直到不再需要工具为止。
...
| `openai.RateLimitError` | 调用频率过高 | 在循环中加 `time.sleep(1)`；检查是否在短时间内多次运行 |
