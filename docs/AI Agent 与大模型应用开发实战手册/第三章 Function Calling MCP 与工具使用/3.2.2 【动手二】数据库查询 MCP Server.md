...
本节结束后，你将能够：
1. 独立实现一个支持 SQLite 和 PostgreSQL 双后端的数据库查询 MCP Server，让 Claude 可以直接理解并查询数据库；
...
# 安装依赖（版本锁定）
uv pip install "mcp[cli]>=1.3.0" "fastmcp>=0.4.0" "sqlparse>=0.5.0" "psycopg2-binary>=2.9.9"

# 可选：PostgreSQL 支持已包含在上面的 psycopg2-binary 中
# 如果只需要 SQLite（无需额外安装，Python 内置）

# 验证安装
python -c "import mcp, fastmcp, sqlparse; print('依赖安装成功')"
```

> **Colab 用户**：`!pip install "mcp[cli]>=1.3.0" "fastmcp>=0.4.0" "sqlparse>=0.5.0"` 即可，SQLite 内置无需额外安装。
...
**目标**：封装支持 SQLite 和 PostgreSQL 双后端的连接工厂，以及统一的查询结果格式化逻辑。
...
| 报错信息 | 原因 | 解决方案 |
|---------|------|---------|
| `ModuleNotFoundError: No module named 'fastmcp'` | 虚拟环境未激活，或依赖未安装 | `source .venv/bin/activate && uv pip install "fastmcp>=0.4.0"` |
| `❌ 数据库文件不存在：data/sample.db` | 未运行建库脚本 | `python scripts/create_sample_db.py` |
