...
# 将上一节训练产出的 checkpoint 路径记下来，后续会用到
# 默认路径：./outputs/qwen2.5-7b-customer-service/final
```

> Colab 用户：`!pip install transformers peft torch rouge-score bert-score openai matplotlib pandas tqdm python-dotenv` 即可
