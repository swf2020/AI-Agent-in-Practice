...
graph TD
    ...
    E3 --> F[工程化封装]
    F --> F1[evaluate_translation<br/>单条评估函数]
...
> Colab 用户：`!pip install litellm scipy pandas matplotlib seaborn python-dotenv pydantic tenacity` 即可，无需创建虚拟环境
...
- ⚠️ 30 条测试集是最小有效规模。低于 20 条，Spearman 相关系数的统计显著性不足。
...
- ⚠️ 注意 V3/V4 中的 `{{` 和 `}}`——这是 Python f-string 中转义大括号的写法，实际发给模型时会变成单个 `{` 和 `}`。如果直接写 `{` 会报 `KeyError`。
...
- `concurrency=10` 是 GPT-4o 的保守并发设置，可根据实际速率限制调整。DeepSeek 可以提高到 20。
- ⚠️ `response_format={"type": "json_object"}` 只有 OpenAI 和部分兼容接口支持。如果用 Claude 作为裁判，需要改为在 Prompt 中强调"只输出 JSON"，然后用 try-except 处理解析失败。
...
### Step 7：工程化封装——对外暴露标准接口

**目标**：将所有逻辑封装为可复用的函数，支持单条调用和批量异步调用。
...
| `RateLimitError: 429` | 并发请求超过 API 速率限制 | 降低 `concurrency` 参数（GPT-4o 建议 ≤10），或在 `judge_batch` 中加 `asyncio.sleep(0.5)` 间隔 |
...
| `ImportError: deep_translator` | Google Translate 依赖未安装 | `uv pip install deep-translator==1.11.4` |
