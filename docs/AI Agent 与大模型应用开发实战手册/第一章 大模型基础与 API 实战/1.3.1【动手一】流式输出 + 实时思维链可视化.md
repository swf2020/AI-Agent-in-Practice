...
1. **模型体系完全重构**：原版基于 OpenAI + Anthropic (Claude) 双供应商，实际代码已统一为 OpenAI 兼容 API 体系，支持 DeepSeek-V3、Qwen-Max、GPT-4o、DeepSeek-Reasoner 四种模型，通过 `core_config.py` 的 `MODEL_REGISTRY` 统一管理。

2. **移除 Anthropic/Claude Extended Thinking**：原版"模式二"使用 Claude 原生的 `extended_thinking` 和 `betas=["interleaved-thinking-2025-05-14"]`，实际代码改为 **DeepSeek 推理模型**，有两种实现路径：
   - `use_reasoner=True`：调用 deepseek-reasoner 模型，通过 `delta.reasoning_content` 字段获取原生推理内容
   - `use_reasoner=False`：调用普通 chat 模型 + `DEEPSEEK_THINKING_PROMPT` 系统提示词引导 `<thinking>` 标签输出

3. **新增 `core_config.py`**：实际代码新增了模型注册表配置模块，包含 4 个模型的完整配置（litellm_id、定价、API Key 环境变量名、base_url），以及 `get_openai_client()` 和 `get_default_model()` 两个工厂函数，实现模型一键切换。

4. **Step 1 使用配置工厂**：`step1_raw_stream.py` 不再直接 `OpenAI()` 和硬编码 `"gpt-4o"`，改为通过 `get_openai_client()` 和 `get_default_model()` 获取。

5. **Web 版模型选择器变化**：原版下拉选项为 gpt-4o / gpt-4o-mini / gpt-4-turbo，实际代码改为 DeepSeek Chat / DeepSeek Reasoner / Qwen / GPT-4o；推理模式从 "Extended Thinking（Claude）" 改为 "DeepSeek 推理模型"。

6. **依赖版本约束放宽**：原版使用锁定版本（`openai==1.78.0` 等），实际 `requirements.txt` 使用最低版本（`openai>=1.0.0` 等）。

7. **环境变量模板更新**：`.env.example` 中不再包含 `ANTHROPIC_API_KEY`，改为 `DEEPSEEK_API_KEY` 和 `DASHSCOPE_API_KEY`。

8. **experiment.py 第三种模式更名**：从 "Claude Extended Thinking" 改为 "DeepSeek 思考模式（`<thinking>` 标签）"。
