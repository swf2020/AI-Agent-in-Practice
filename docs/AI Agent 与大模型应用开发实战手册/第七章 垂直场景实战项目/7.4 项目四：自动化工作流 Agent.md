...
> Colab 用户：`!pip install langgraph==0.2.55 langchain-anthropic==0.3.0 celery==5.4.0 redis==5.2.0 fastapi==0.115.6 uvicorn==0.32.1 httpx==0.27.2 pydantic==2.10.3 google-api-python-client==2.156.0 slack-sdk==3.33.4 python-dotenv==1.0.1`
...
SLACK_APPROVAL_CHANNEL=C_CHANNEL_ID   # 发送审批请求的频道
SLACK_NOTIFY_CHANNEL=C_NOTIFY_ID      # 发送结果通知的频道

# Notion（可选，与 Jira 二选一）
...
class RiskLevel(str, Enum):
    """任务风险等级，决定是否需要人工审批"""
    LOW = "low"     # 直接执行：创建任务、发通知
    HIGH = "high"   # 需要审批：删除、批量操作、对外发送
...
class WorkflowState(BaseModel):
    ...
    # 审批状态
    approval_message_ts: Optional[str] = None  # Slack 消息时间戳，用于更新消息
...
def _get_gmail_service():
    ...
    # Credentials 会自动用 refresh_token 获取 access_token
    creds.refresh(Request())
    return build("gmail", "v1", credentials=creds)


def _decode_body(payload: dict) -> str:
    """递归解析邮件 body，支持 multipart"""
    if payload.get("body", {}).get("data"):
        return base64.urlsafe_b64decode(payload["body"]["data"]).decode("utf-8", errors="replace")
    # multipart：取第一个 text/plain part
...
def gmail_read_email(message_id: str) -> str:
    ...
    Returns:
        JSON 字符串，包含 subject、sender、body、received_at
    """
    ...
    email = EmailMessage(
        ...
        body=body[:4000],  # 截断保护，防止 body 过长超出 context window
...
def gmail_mark_processed(message_id: str) -> str:
    """
    给邮件添加 'WORKFLOW_PROCESSED' 标签并移除 UNREAD 标签。
    避免同一封邮件被重复处理。
    Args:
        message_id: Gmail 邮件 ID
    Returns:
...
def slack_send_notification(message: str, channel: Optional[str] = None) -> str:
    ...
    发送普通文本通知到 Slack 频道。
    Args:
        message: 通知内容（支持 Slack mrkdwn 格式）
        channel: 目标频道 ID，默认使用配置中的通知频道
    Returns:
        消息时间戳 ts（可用于后续更新消息）
    """
...
def send_approval_request(
    ...
    """
    发送带 Approve/Reject 按钮的审批请求消息。
    注意：这个函数不是 @tool，因为它由 Agent 节点直接调用而非 LLM 调用。
    workflow_run_id 编码在 action_id 中，Slack 回调时用于恢复对应的 Agent 实例。

    Returns:
        消息 ts，存入 WorkflowState.approval_message_ts
    """
    blocks = [
        {
            "type": "header",
            ...
            "elements": [
                {
                    ...
                    "style": "primary",
                    # action_id 格式：approve_{run_id}，Slack 回调时解析 run_id
    ...
    response = _client.chat_postMessage(
        ...
        text=f"审批请求：{task_title}",  # fallback text
        blocks=blocks,
    )
    return response["ts"]
...
def notion_create_task(
    ...
    if assignee:
        # Notion People 属性需要 Notion User ID，实际项目中需要维护 email→user_id 映射表
        # 这里用 rich_text 降级处理，生产中建议接 Notion User API
...
def jira_create_issue(
    ...
    Args:
        ...
        assignee: Jira 用户名或账户 ID（可选）
        due_date: 截止日期，格式 YYYY-MM-DD（可选）
        priority: Jira 优先级 Lowest/Low/Medium/High/Highest
    Returns:
...
**目标**：用 LangGraph 把上述工具串联成有状态的执行图，核心是 `__interrupt__` 实现人工审批暂停点。
...
from langgraph.checkpoint.redis import RedisSaver  # 用 Redis 持久化 checkpoint
...
# LLM 初始化：任务提取用 claude-3-5-sonnet，速度和质量均衡
_llm = ChatAnthropic(
    model="claude-3-5-sonnet-20241022",
    api_key=settings.anthropic_api_key,
    temperature=0,  # 信息提取任务，temperature=0 保证输出稳定
)

# 绑定结构化输出，强制 LLM 返回 ExtractedTask 格式
...
def node_extract_task(state: WorkflowState) -> dict:
    """
    调用 LLM 从邮件内容提取结构化任务。
    使用 structured_output 而非 Function Calling，避免模型在 JSON 格式上绕弯路。
    """
...
def node_request_approval(state: WorkflowState, config: dict) -> dict:
    """
    发送 Slack 审批请求，然后调用 interrupt() 暂停 Agent。
    interrupt() 会抛出一个特殊异常，LangGraph 捕获后将 state 持久化到 checkpoint，
    等待外部调用 graph.update_state() 注入审批结果后继续执行。
    """
    assert state.extracted_task is not None and state.email is not None

    # config["configurable"]["thread_id"] 是当前 run 的唯一标识
    ...
    # interrupt() 后面的代码在本次执行中不会运行
    # LangGraph 会将 state（含 approval_message_ts）保存到 Redis checkpoint
    # 当 Slack 回调触发 graph.invoke(Command(resume=...)) 时，从这里继续
    approval_result: dict = interrupt({"message": "等待人工审批", "ts": ts})

    # ← 恢复执行后从这里继续，approval_result 来自 Command(resume=approval_result)
    approved: bool = approval_result.get("approved", False)
    operator: str = approval_result.get("operator", "unknown")

    # 更新 Slack 原消息，替换按钮为结果
...
def node_write_task(state: WorkflowState) -> dict:
    """将任务写入 Notion 或 Jira（优先 Notion，无配置则 Jira）"""
...
def build_workflow_graph(redis_url: str) -> "CompiledGraph":
    ...
    redis_url 用于 checkpoint 持久化，是 interrupt 能工作的前提。
    """
    ...
    # Redis checkpoint：interrupt 依赖 checkpoint 持久化当前状态
...
app.conf.update(
    ...
    enable_utc=True,
    # 任务超时 10 分钟（含 LLM 调用 + 人工审批等待不在此列，审批通过后续由回调触发）
    task_soft_time_limit=600,
    task_time_limit=660,
)

# Celery Beat 定时任务：每 5 分钟轮询一次 Gmail
...
def _fetch_unprocessed_emails() -> list[str]:
    """从 Gmail 获取未贴 WORKFLOW_PROCESSED 标签的邮件 ID 列表"""
    creds = Credentials(
        token=None,
        refresh_token=settings.gmail_refresh_token,
        client_id=settings.gmail_client_id,
    ...
    result = service.users().messages().list(
        ...
        maxResults=10,  # 单次最多处理 10 封，防止突发大量邮件阻塞队列
    ).execute()
...
def process_single_email(self, email_id: str) -> dict:
    """
    处理单封邮件的 Celery 任务。
    thread_id = email_id：确保同一封邮件的所有执行（首次 + 审批恢复）使用同一 checkpoint。
    """
    try:
        graph = build_workflow_graph(settings.redis_url)
        initial_state = WorkflowState(email_id=email_id)

        # thread_id 使用 email_id，保证同一封邮件的审批恢复能找到正确 checkpoint
...
def poll_gmail_and_dispatch() -> dict:
    ...
    定时轮询任务：拉取未处理邮件并逐一入队。
    轮询 vs Webhook 的工程取舍：
    - Webhook 延迟 < 1s，但需要 HTTPS 公网端点和 Gmail Push API 申请
    - 轮询延迟 = 轮询间隔（5min），部署简单，适合 MVP 和内网环境
    两者并不互斥，生产中 Webhook 为主、轮询作为保底兜底
    """
...
async def gmail_push_webhook(request: Request):
    """
    Gmail Pub/Sub Push Notification 端点。
    配置方式：
    1. 创建 Google Cloud Pub/Sub Topic
    2. Gmail API watch() 订阅该 Topic
    3. 将此端点设为 Push 订阅 URL（需 HTTPS，本地开发用 ngrok）
    """
    body = await request.json()

    # Gmail 消息通过 base64 编码嵌在 Pub/Sub 消息里
    ...
    # 实际生产中需要用 historyId 调用 Gmail History API 获取新邮件列表
    # 这里简化为直接触发轮询，确保新邮件被捕获
    from scheduler.tasks import poll_gmail_and_dispatch
    poll_gmail_and_dispatch.delay()

    # 必须返回 200，否则 Pub/Sub 会一直重试
    return {"status": "ok"}
```
...
async def slack_interactions_handler(request: Request):
    """
    Slack Interactive Components 回调端点。
    路由需在 app.py 中注册：app.post("/slack/interactions")(slack_interactions_handler)
    """
    # 1. 验证 Slack 签名，防止伪造请求
    ...
    # 从 action_id 解析出 run_id 和操作类型
    # action_id 格式：approve_{run_id} 或 reject_{run_id}
    ...
    # 2. 用 run_id（= email_id = thread_id）恢复对应的 Agent
    graph = build_workflow_graph(settings.redis_url)
    config = {"configurable": {"thread_id": run_id}}

    # Command(resume=data) 将数据注入到 interrupt() 的返回值，继续执行图
    graph.invoke(
        Command(resume={"approved": approved, "operator": operator}),
        config=config,
    )

    # 返回空响应，Slack 会自动关闭 loading 状态
    return {"ok": True}


async def _verify_slack_signature(request: Request) -> None:
    """验证请求来自 Slack，防止伪造回调攻击"""
...
from langgraph.checkpoint.memory import MemorySaver  # 测试用内存 checkpoint
from langgraph.types import Command
...
def run_smoke_test():
    ...
    with (
        ...
        # 使用内存 checkpoint 替代 Redis，便于本地测试
        from langgraph.graph import StateGraph, END
        # 直接构建图，注入 MemorySaver
        from langgraph.checkpoint.memory import MemorySaver
        import agent.workflow_graph as wg

        original_build = wg.build_workflow_graph

        def patched_build(redis_url):
            builder = __import__("langgraph.graph", fromlist=["StateGraph"]).StateGraph(WorkflowState)
        ...
        try:
            ...
            print(f"  图执行状态：{type(e).__name__}（中断等待审批，符合预期）")

        # 检查 checkpoint 状态
        snapshot = graph.get_state(config)
        print(f"  当前节点：{snapshot.next}")
        print(f"  风险等级：{snapshot.values.get('extracted_task', {}).get('risk_level', 'N/A') if isinstance(snapshot.values.get('extracted_task'), dict) else 'extracted'}")

        print("\n▶ Step 2: 模拟 Slack Approve 回调...")
        result = graph.invoke(
            Command(resume={"approved": True, "operator": "alice"}),
            config=config,
        )
        print(f"  执行结果：{result}")
        print(f"  Notion 页面 ID：{result.get('notion_page_id', 'N/A')}")

        print("\n✅ 冒烟测试通过！")

        # 恢复原函数
        wg.build_workflow_graph = original_build
...
▶ Step 1: 首次执行（预期在审批节点暂停）...
  图执行状态：GraphInterrupt（中断等待审批，符合预期）
  当前节点：('request_approval',)
  风险等级：extracted

▶ Step 2: 模拟 Slack Approve 回调...
  执行结果：{'email_id': 'test_msg_001', 'email': ..., 'approved': True, ...}
  Notion 页面 ID：notion-page-abc
...
| `RedisSaver` 导入失败：`ModuleNotFoundError` | 缺少 `langgraph-checkpoint-redis` 包 | `uv pip install langgraph-checkpoint-redis==0.0.6`；本地测试可改用 `MemorySaver` |
