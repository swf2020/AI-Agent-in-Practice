...
1. **step4_query.py - LLM 调用方式全面重构**：原文档使用 `openai.OpenAI` 客户端，实际代码已迁移到 `litellm` + `core_config` 架构。新增了 `get_llm_client()` 函数，通过 `MODEL_REGISTRY` 支持 DeepSeek 和 Qwen 双模型切换。`RAGPipeline` 的 `__init__` 新增 `model_key` 参数，默认模型从 `"gpt-4o-mini"` 改为 `"DeepSeek-V3"`。`ask()` 方法从 `self.llm.chat.completions.create()` 改为 `self.llm_client.completion()`。

2. **step4_query.py - 检索策略升级**：`retrieve()` 方法从 `qdrant.search()` 改为 `qdrant.query_points()`，新增 `_retrieve_full_scan()` 全量扫描降级机制，当索引查询无结果时自动 fallback。

3. **step4_query.py - 环境变量移除 OpenAI 依赖**：不再使用 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL` 环境变量，改为通过 `core_config.py` 的 `MODEL_REGISTRY` 管理。

4. **step3_index.py - 索引优化**：`ensure_collection()` 新增 `client.update_collection()` 调用，设置 `indexing_threshold: 1` 确保小规模数据也能立即触发索引构建。

5. **app.py - 阈值调整**：`score_threshold` 从 `0.5` 改为 `0.4`。

6. **新增 main.py**：统一命令行入口，提供 `index`、`ask`、`chainlit` 三个子命令。

7. **新增 core_config.py**：模型注册表，支持 DeepSeek-V3 和 Qwen-Max 两种模型。

8. **requirements.txt - 依赖变化**：移除了 `openai`，新增了 `litellm` 和 `numpy`；`markitdown` 版本从 `0.0.1a3` 改为 `[docx,pptx]>=0.1.0`；其他依赖版本约束也有所放宽。

9. **.env 配置变化**：不再使用 `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL`，改为 `DEEPSEEK_API_KEY` / `DASHSCOPE_API_KEY`。
