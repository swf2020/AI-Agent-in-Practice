...
> Colab 用户：`!pip install qdrant-client sentence-transformers pypdf markitdown openai chainlit python-dotenv tiktoken langchain-text-splitters` 即可，无需创建虚拟环境。

新建 `.env` 文件管理密钥：

```bash
OPENAI_API_KEY=sk-xxx          # 若用 Ollama 本地模型可留空
OPENAI_BASE_URL=               # 用 Ollama 时填 http://localhost:11434/v1
OPENAI_MODEL=gpt-4o-mini       # 或 ollama 模型名，如 qwen2.5:7b
```
...
- ⚠️ pypdf 对扫描版 PDF 无效，这是生产环境最常见的坑之一。建议在解析前做一次页面文字密度检测，低于阈值则提示用户。
...
- ⚠️ `chunk_overlap` 不是越大越好：过大会导致相邻块高度重复，Top-K 检索时同质化结果占满名额，反而降低召回多样性。经验值：`overlap ≈ chunk_size × 10-15%`。
...
- ⚠️ **重复索引问题**：`upsert` 会基于 ID 更新，但我们用累加 ID，多次运行同一文档会产生重复数据。生产环境应先按 `source` 字段删除旧数据再重建，或改用文件哈希作为 ID。
...
**关键点**：
- `score_threshold=0.5` 是关键防线：余弦相似度低于 0.5 的结果通常和问题关联很弱，强行注入 Prompt 反而会干扰 LLM 作答。实际阈值需根据你的模型和数据调整，可先用 0.4-0.6 区间实验。
...
| `openai.AuthenticationError` | API Key 未配置或无效 | 检查 `.env` 文件；使用 Ollama 时 `OPENAI_API_KEY=ollama` |
...
> ⚠️ **生产注意**：本节实现的 Qdrant 使用本地文件存储模式，不支持多进程并发写入。生产环境应改为 Docker 独立部署 Qdrant（`docker run -p 6333:6333 qdrant/qdrant`），并通过网络连接（`QdrantClient(host="localhost", port=6333)`）访问，支持并发读写与数据持久化管理。
