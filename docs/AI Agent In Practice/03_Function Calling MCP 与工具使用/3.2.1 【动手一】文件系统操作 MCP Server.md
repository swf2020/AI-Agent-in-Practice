...
本文档已根据实际代码重写。与上一版文档相比，主要差异如下：

| 项目 | 原文档描述 | 实际代码 |
|------|-----------|---------|
| `read_file` | 接受 `encoding` 参数；返回错误字符串；返回内容带元信息头部 | 不接受 `encoding` 参数（固定 utf-8）；抛出 `FileNotFoundError` / `ValueError` 异常；返回纯文本内容 |
| `write_file` | 有 `overwrite` 参数（默认 False）；返回含 `action`/`char_count`/`success` 字段的字典 | 无 `overwrite` 参数；返回含 `status`/`bytes_written` 字段的字典 |
| `list_directory` | `max_depth` 默认 2，上限 4；过滤 `.git`/`__pycache__` 等；返回含 `truncated`/`summary`（目录数+文件数） | `max_depth` 默认 1，无上限；不过滤任何目录；`summary` 仅含总条目数 |
| `search_files` | 参数名 `keyword`；搜索**文件内容**含上下文；返回 `matches` 列表含行号和上下文 | 参数名 `query`；仅搜索**文件名**（`query in fname`）；返回 `files` 路径列表 |
| `get_file_info` | 不存在时返回错误字典；返回 `readable`/`within_limit`/`suffix`/`type` 字段 | 不存在时抛出 `FileNotFoundError`；返回 `is_file`/`is_dir`/`path` 字段 |
| `ALLOWED_ROOT` | 模块加载时固定的 `Path` 常量 | 通过 `_get_allowed_root()` 动态获取，运行时环境变量修改有效 |
| `MAX_FILE_SIZE_BYTES` | 在 `ALLOWED_ROOT` 之后定义 | 在文件开头第一个定义 |
| `_build_tree` | 内嵌在 `list_directory` 中返回 `dict` | 独立的辅助函数，返回 `list` |
| 启动入口 | 内嵌在 `filesystem_server.py` 底部 | 同时在 `filesystem_server.py` 和 `main.py` 中各有入口 |
| `main.py` | 文档未提及 | 实际存在，加载 `.env` 后导入 `mcp` 和 `ALLOWED_ROOT` |
| `requirements.txt` | 文档中安装 `mcp[cli]==1.9.2` + `fastmcp==2.3.4` | 实际需要 `fastmcp>=2.0.0`, `python-dotenv>=1.0.0`, `pytest>=7.0.0` |
| `test_server.py` | 搜索 `keyword="MCP"`, `file_pattern="*.md"` | 实际搜索 `query="MCP"`, `file_pattern="*.txt"` |
