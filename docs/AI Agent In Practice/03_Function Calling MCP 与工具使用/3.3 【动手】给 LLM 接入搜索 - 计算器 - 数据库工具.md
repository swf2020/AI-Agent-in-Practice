...
本次重写对照实际代码发现以下主要差异并修正：

1. **调用层从 OpenAI SDK 切换为 LiteLLM**：实际 `agent.py` 使用 `litellm.completion()` 而非 `openai.OpenAI().chat.completions.create()`，配合 `core_config.py` 统一管理模型配置。这意味着文档中的依赖从 `openai` 为主变为 `litellm` 为主（requirements.txt 中两者都有，但运行时只用 litellm）。

2. **工具 API Key 获取方式改为优雅降级**：`TavilySearchTool` 和 `E2BCodeExecutorTool` 实际代码使用 `os.environ.get()` 而非 `os.environ[]`，并在 `run()` 方法开头检查 API Key 是否存在，返回友好提示而非直接抛 `KeyError`。

3. **E2BCodeExecutorTool 使用 context manager**：实际代码使用 `with Sandbox(...) as sandbox:` 确保资源释放，文档中原版本缺少 `with` 语句。

4. **E2BCodeExecutorTool 输出拼接方式**：实际代码用 `"\n".join(output_parts)`，文档中原为 `"\n\n".join(output_parts)`。

5. **db_tool 截断提示无 emoji**：实际代码为 `"结果已截断至..."`，文档中原有 `⚠️` emoji。

6. **agent.py verbose 输出无 emoji**：实际代码输出 `"调用工具: {tool_name}"`，文档中为 `"🔧 调用工具: {tool_name}"`。

7. **新增 core_config.py**：文档中缺少此文件的内容，现已补充 Step 0 完整说明。

8. **新增 main.py**：文档中缺少交互式对话入口代码，现已补充。

9. **build_agent 不再硬编码模型**：实际代码中 `build_agent()` 不传 `model` 参数，使用 `core_config` 的默认模型，文档中原为 `model="gpt-4o"`。

10. **ToolCallingAgent.__init__ 的 model 参数类型**：实际为 `model: str | None = None`（可选），文档中为 `model: str = "gpt-4o"`（必填且有默认值）。
