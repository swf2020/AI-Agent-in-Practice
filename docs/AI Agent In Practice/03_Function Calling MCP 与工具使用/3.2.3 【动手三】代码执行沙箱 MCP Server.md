...
### Step 3.5：动态安装包 `install_package`（内部函数）

**目标**：在沙箱环境中通过 `pip install` 动态安装包——被 Step 4 的 `pip_install` MCP Tool 调用。注意此函数需要在 MCP 工具定义之前声明，因为 `pip_install` 会调用它。
