...
本次对比实际代码与原文档，发现以下差异并已修正：

1. **新增 core_config.py 和 main.py**：原文档未提及这两个文件。实际代码通过 `core_config.py` 的 `MODEL_REGISTRY` 统一管理模型配置（支持 DeepSeek-V3、Qwen-Max、Claude-Max 三种模型），`agent/workflow_graph.py` 中使用 `get_litellm_id()` 和 `get_api_key()` 替代了硬编码的 `claude-3-5-sonnet-20241022` 和 `settings.anthropic_api_key`。`main.py` 是 FastAPI 服务的启动入口。

2. **config.py 所有字段均有默认值 `= ""`**：原文档中 `anthropic_api_key: str` 没有默认值，实际代码为 `str = ""`，所有配置字段均为可选。

3. **agent/workflow_graph.py 使用 `RunnableConfig`**：原文档中 `node_request_approval` 的 `config` 参数类型为 `dict`，实际代码使用 `langgraph.config.RunnableConfig`。

4. **工具函数 docstring 简化**：原文档中 `slack_send_notification`、`gmail_read_email`、`gmail_mark_processed`、`notion_create_task`、`jira_create_issue` 等工具函数有多行详细 docstring，实际代码中均为一行简短描述。`send_approval_request` 和 `update_approval_message` 在实际代码中没有 docstring。

5. **models.py 移除了内联注释**：原文档中 `RiskLevel` 枚举值和 `WorkflowState` 字段有详细中文注释，实际代码中无这些注释。

6. **smoke_test.py 大幅重构**：
   - Mock 方式从 `patch("xxx.invoke")` 改为 `MagicMock` 替换整个 `@tool` 对象，避免 Pydantic `__setattr__` 校验问题
   - 移除了对 `build_workflow_graph` 的 patch，改为 patch `settings`
   - 测试操作人从 `"alice"` 改为 `"admin"`
   - 输出格式简化，移除了风险等级显示和 `notion_page_id` 外的其他字段
   - 不再恢复原始 `build_workflow_graph` 函数

7. **requirements.txt 版本约束改为 `>=`**：原文档使用精确版本 `==`，实际代码使用最小版本 `>=`。新增 `langgraph-checkpoint-redis>=0.3.0`、`litellm>=1.40.0`、`pytest>=7.0.0`。

8. **.env.example 移除了部分注释**：原文档中 `# 见 Step 2 获取方式` 等说明性注释在实际文件中不存在，`SLACK_APPROVAL_CHANNEL` 和 `SLACK_NOTIFY_CHANNEL` 无行尾注释。
