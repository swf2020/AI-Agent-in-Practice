...
本文档基于 `libs/` 中的实际代码重写，与原文档的主要差异如下：

| 差异项                           | 原文档描述                                                                                        | 实际代码实现                                                               |
| ----------------------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **文件结构**                      | 描述多个独立文件（`data_prep.py`、`model_setup.py`、`train.py`、`analyze_training.py`、`export_model.py`） | 实际只有 4 个文件：`main.py`、`finetune.py`、`smoke_test.py`、`core_config.py`  |
| **训练配置**                      | 使用 `SFTConfig` + `EarlyStoppingCallback` + `eval_strategy` + DeepSpeed ZeRO-2                | 实际使用 `TrainingArguments`，无验证集评估、无早停、无 DeepSpeed 支持                   |
| **数据集处理**                     | 有完整的数据清洗、去重、ChatML 转换流水线（`build_dataset()`）                                                  | 实际通过 `load_dataset()` 加载外部 HuggingFace 数据集，用 `format_sample()` 简单格式化 |
| **模型导出**                      | 描述了 GGUF/ONNX 转换、Ollama 部署全流程                                                                | 实际代码中无模型导出逻辑                                                         |
| **训练监控**                      | 有 `analyze_training.py` 绘制 Loss 曲线                                                           | 实际无此模块                                                               |
| **激活模型**                      | 原文档未明确                                                                                       | 实际 `ACTIVE_MODEL_KEY = "Qwen-Max"`                                   |
| `MAX_SEQ_LENGTH`              | 1024                                                                                         | 512                                                                  |
| `fp16/bf16`                   | `bf16=True`                                                                                  | `fp16=True`                                                          |
| `per_device_train_batch_size` | 2                                                                                            | 4                                                                    |
| 保存策略                          | `save_strategy="steps", save_steps=50`                                                       | `save_strategy="epoch"`                                              |
