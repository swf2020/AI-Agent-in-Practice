...
本文档相对于旧版代码和文档做了以下更新以匹配当前实际代码：

1. **重排序方案变更**：从本地 `BGE-Reranker Cross-Encoder`（`sentence-transformers`）改为 **DashScope qwen3-rerank API**（`httpx` 调用）。优势是无需下载模型、无 GPU 依赖，首次运行即可使用。代码中保留了降级策略——API 失败时回退到向量检索原始分数排序。

2. **Embedding 方案变更**：从 OpenAI `text-embedding-3-small`（1536 维）改为 DashScope `text-embedding-v4`（1024 维），使用独立的 `embed_client`。

3. **新增 `core_config.py` 统一配置**：所有模型名、API Key、base_url 不再硬编码，而是通过 `core_config` 模块统一管理。聊天模型注册表包含 3 个可选模型（DeepSeek-V3、Qwen-Max、OpenAI-GPT-4o-mini），默认激活 DeepSeek-V3。

4. **双客户端架构**：`NaiveRAG` 类现在使用 `embed_client`（DashScope Embedding）和 `chat_client`（可切换的聊天模型）两个独立客户端。

5. **Qdrant API 更新**：`qdrant.search()` 已更新为 `qdrant.query_points()`（适配新版 qdrant-client）。

6. **硬编码模型常量移除**：`NaiveRAG` 类中的 `EMBED_MODEL`、`CHAT_MODEL`、`EMBED_DIM` 类常量已移除，改为从 `core_config` 动态获取。

7. **新增 `main.py`**：项目新增了快速演示入口 `main.py`，使用精简文档集展示完整的 Advanced RAG 查询流程。

8. **模型参数默认值变更**：`QueryRewriter.__init__` 和 `SummaryCompressor.__init__` 的 `model` 参数默认值从硬编码 `"gpt-4o-mini"` 改为 `None`（从 `core_config.get_chat_model_id()` 获取）。`DashScopeReranker.__init__` 同样使用配置默认值。
