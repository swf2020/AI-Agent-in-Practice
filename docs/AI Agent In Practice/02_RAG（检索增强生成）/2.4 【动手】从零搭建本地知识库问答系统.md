...
核心学习点（3 个）：
1. **文档解析与切块的工程取舍**：不同格式的解析工具选择，以及两种切块策略的效果差异；
...
# ── 策略 2：章节切块 ──────────────────────────────────────
def chunk_by_section(doc: ParsedDocument, max_chunk_size: int = 1000) -> list[Chunk]:
    """
    章节切块：利用 Markdown 标题层级（# ## ###）将文档按章切割。

    为什么这样做？
    技术文档或白皮书通常有清晰的章节结构，按章节切块
    能保持语义完整性，避免在段落中间切割。
    """
    enc = _ENCODER  # 使用缓存的 encoder
...
**关键点**：
- 两种策略的选型建议：**结构化文档（Word/PPT）用章节切块**，语义最完整；**PDF 和网页用固定大小切块**，因为提取出的文本往往缺少层级信息。语义切块（按句子相似度聚合）效果最好但耗时显著，教学版暂不收录，可作为扩展练习。
...
- **幂等索引**：使用 `_chunk_id()` 基于来源+索引的 MD5 hash 生成 Point ID，多次运行同一文档不会重复写入，`upsert` 会覆盖相同 ID 的数据。
...
def get_llm_client(model_key: str = "DeepSeek-V3"):
    ...
    不再污染全局 os.environ，返回 api_key 和 api_base，
    ...
    # 统一返回 litellm、model_id、api_key、base_url
...
class RAGPipeline:
    ...
    def _retrieve_full_scan(self, query_vector: list[float]) -> list[RetrievedChunk]:
        ...
        使用分页 scroll 获取全部数据，避免硬编码 1000 条上限遗漏数据。
        """
        import numpy as np
...
请用中文回答："""
    ...
    def ask(self, question: str) -> RAGAnswer:
        ...
        # 显式传入 api_key 和 api_base，避免污染全局环境变量
        completion_kwargs = {
...
python main.py index ./docs/my_report.pdf --strategy fixed

# 问答查询
python main.py ask "pathlib 是什么？"
python main.py ask "pathlib 的优势" --top_k 3 --threshold 0.4
