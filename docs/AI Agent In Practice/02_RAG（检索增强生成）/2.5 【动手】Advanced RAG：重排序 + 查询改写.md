...
class RetrievedChunk:
    ...
    score 改为 float | None，压缩后的上下文无真实评分。
    """
    text: str
    score: float | None
    chunk_id: str
...
class QueryRewriter:
    ...
    def hyde(self, query: str) -> str:
        ...
        # 字数上限保护，避免 LLM 不遵守 prompt 导致超长噪声
...
class DashScopeReranker:
    ...
    def rerank(
        ...
        Returns:
            ...
            注意：返回的是新 RetrievedChunk 对象，不会修改传入的原始数据。
        """
        if not chunks:
            return []
...
class AdvancedRAG(NaiveRAG):
    ...
    def advanced_query(self, question: str) -> dict:
        ...
        if cfg.use_compression and candidates:
            ...
            # score 设为 None 表示压缩后未经真实评分，避免误导下游逻辑
            candidates = [
                RetrievedChunk(
