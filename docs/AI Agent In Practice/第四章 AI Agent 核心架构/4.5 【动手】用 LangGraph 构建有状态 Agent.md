...
- `TypedDict` 而非 `dataclass`：LangGraph 要求 State 是可序列化的 dict 结构，便于 Checkpoint 持久化。
...
本文档是根据实际代码仓库更新后的版本，与原始文档相比存在以下重要差异：

1. **新增 `core_config.py` 模型注册表**：实际代码中 `agent.py` 不再硬编码模型名（如 `"claude-sonnet-4-5"`），而是通过 `core_config.py` 的 `MODEL_REGISTRY` 统一管理。模型 ID 从 `core_config` 的 `get_litellm_id()` / `get_chat_model_id()` 获取，API Key 从 `get_api_key()` 获取。

2. **`agent.py` 的 `create_llm()` 签名变更**：默认参数从 `provider="anthropic"` 改为 `provider="default"`，新增第三种路由方式——通过 `langchain_litellm.ChatLiteLLM` 走 LiteLLM 适配层（由 `core_config.ACTIVE_MODEL_KEY` 控制具体模型）。同时直连 Anthropic/OpenAI 时也从 `core_config` 获取模型名而非硬编码。

3. **`tools.py` 优雅降级机制**：原始代码直接使用 `langchain_community.tools.tavily_search.TavilySearchResults`，实际代码改用 `langchain_tavily.TavilySearch`，并增加了 `mock_search` 工具作为 TAVILY_API_KEY 未配置时的 fallback。`.env` 加载逻辑也支持向上查找。

4. **`run_agent.py` 的 stream input 变更**：原始文档中第一轮使用条件表达式且只传 `messages` 字段，实际代码中每次 stream 都传入完整的 `{"messages": [...], "tool_calls_count": 0, "requires_approval": False}`，避免 State 字段缺失。

5. **`checkpoint_demo.py` 入口变更**：原始文档的 `if __name__ == "__main__"` 只调用 `demo_multi_turn_memory()`，实际代码同时调用 `demo_multi_turn_memory()` 和 `demo_human_approval()`。

6. **依赖版本更新**：`requirements.txt` 中各依赖已升级到较新版本（如 `langgraph>=1.1.0`），新增 `langchain-litellm` 和 `langchain-tavily` 包。

7. **`.env.example` 变更**：不再包含 `OPENAI_API_KEY` 和 `ANTHROPIC_API_KEY`，改为 `DEEPSEEK_API_KEY` 和 `DASHSCOPE_API_KEY`（与 `core_config.py` 中的模型注册表一致）。
