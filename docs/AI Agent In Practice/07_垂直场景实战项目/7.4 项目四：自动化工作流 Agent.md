...
from config import settings
from core_config import get_chat_model_id, get_api_key  # [Fix #2] ChatAnthropic 使用无前缀模型名

_api_key = get_api_key() or (settings.anthropic_api_key or "")
# 注意：本项目 Agent 主流程设计为搭配 Claude 使用；ChatAnthropic 需要的
# chat_model_id 是不带 provider 前缀的模型名（如 "claude-3-5-sonnet-latest"）。
# 若切换 ACTIVE_MODEL_KEY 为 DeepSeek/Qwen，需同步将 ChatAnthropic 更换为
# ChatOpenAI，并相应调整 model 参数。
_llm = ChatAnthropic(
    model=get_chat_model_id(),  # [Fix #2] 无前缀，与 ChatAnthropic 兼容
    api_key=_api_key,
    temperature=0,
)
