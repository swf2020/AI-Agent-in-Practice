...
本文档根据实际 Python 源代码进行了以下修正：

1. **分析师数量**：原文档描述使用四个分析师（基本面、情绪、新闻、技术），但实际代码中 `selected_analysts=["fundamentals", "news"]` 仅启用两个。已在文档中补充说明。
2. **决策输出结构**：原文档中的 `TraderDecision` Pydantic 模型（含 `position_size`、`stop_loss_pct`、`take_profit_pct`、`primary_reasoning`、`key_risks` 等字段）与实际代码返回的简单字典（`action`、`reasoning`、`confidence`，以及可能的 `target_price`、`stop_loss`、`take_profit`）不一致。已更新为实际代码结构。
3. **版本兼容处理**：实际代码对 `graph.propagate()` 的返回值做了字符串/字典两种类型的兼容处理，原文档未提及。已补充。
4. **Checkpoint 实现方式**：原文档暗示使用 LangGraph 内置 Checkpoint，但实际代码因 `tradingagents` 0.3.1 不支持 `SqliteSaver` 注入，改用手动 SQLite 方案。已更正。
5. **A 股工具注入**：原文档未提及 `patch_astock_tools()` 在 0.3.1 中为占位函数（no-op）。已补充版本限制说明。
6. **补充文档**：项目 libs/ 下包含两份补充文档——`SUPPLEMENTARY_环境搭建与数据接入.md`（更详细的环境配置指南）和 `SUPPLEMENTARY_动手实验.md`（四个实验的完整操作手册），内容比本文档内嵌子节更详细，适合初次上手的读者参考。
7. **风险偏好映射**：原文档描述三种风险偏好的"实现机制"为系统提示词阈值，实际代码是通过 `reasoning_effort` 参数传递。已更正。
8. **模型对比定价**：原文档未提及实验四中的具体模型定价对比，已补充 GPT-4o 与 DeepSeek-V3 的价格对比表。
9. **编号统一**：本文档已包含 7.1.1 至 7.1.7 完整子节（内嵌），不再依赖独立文件。原独立文件 `7.1.2.md` 和 `7.1.4.md` 已移至 libs/ 项目目录作为补充材料，从 mkdocs nav 中移除。
